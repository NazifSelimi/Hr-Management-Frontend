...
import { Card, Spin, Typography, Button, Table, Modal, Form, Select, message, Row, Col, Input } from "antd";
...
const EmployeeDetails: React.FC = () => {
    ...
    useEffect(() => {
        const fetchEmployee = async () => {
            try {
                const response = await axiosInstance.get<User>(`/employees/${id}`);
    ...
    const handleAssignProjects = async (values: { project_ids: string[], role: string }) => {
        ...
        try {
            ...
            setEmployee((prev) => ({
                ...prev!,
                projects: [...(prev?.projects || []), ...projects.filter(project => values.project_ids.includes(project.id))],
            }));
        } catch (error: any) {
    ...
    const projectColumns = [
        ...
        },
        {
            ...
            render: (_: any, record: { id: string }) => (
                <Button type="link" onClick={() => handleAssignProjects({ project_ids: [record.id], role: 'Default Role' })}>
                    Assign
                </Button>
            ),
        },
    ...
    return (
        <div>
            ...
            <Card>
                ...
                <Button type="primary" onClick={() => setAssignModalVisible(true)}>Assign Projects</Button>
            </Card>
